...
- For slide deck presentations (when `--slides` flag is present or `/generate-slides` is invoked): read `./templates/slide-deck.html` and `./references/slide-patterns.md`
  ...
  The `slide-deck.html` template uses the **Electric Navy** preset (Poppins Black, `#0e2841` navy, `#fbfe7a` yellow-lime accent). Refer to `./references/slide-patterns.md` "Curated Presets" for all five presets (Midnight Editorial, Warm Signal, Terminal Mono, Swiss Clean, Electric Navy).
...
**For embedding interactive HTML content** (LikeC4 architecture diagrams, Storybook, Swagger UI, or any self-contained HTML page), read `./references/likec4-embed.md`. Covers the `slide--embed` slide type with lazy-loaded iframes, controls, fallback, and both local-export and live-server approaches. For **CSP-blocked sites** (GitHub Codespaces, vscode.dev) that prevent iframe embedding, see the **Popup Embed Slide** pattern in `./references/slide-patterns.md`.
...
**Before generating slides**, read `./references/slide-patterns.md` (engine CSS, slide types, transitions, nav chrome, presets) and `./templates/slide-deck.html` (reference template showing all 10 types). Also read `./references/css-patterns.md` for shared patterns, `./references/libraries.md` for Mermaid/Chart.js theming, and `./references/likec4-embed.md` for embedding interactive HTML content (LikeC4, Storybook, etc.).
...
**Curated presets:** Five slide-specific presets as starting points (Midnight Editorial, Warm Signal, Terminal Mono, Swiss Clean, Electric Navy) plus the existing 8 aesthetic directions adapted for slides. Pick one and commit. See `slide-patterns.md` for preset CSS values.
...
<head>
  ...
  <style>
    ...
    /* Skip link, focus-visible, sr-only, prefers-contrast styles */
  </style>
</head>
<body>
  ...
  <main id="main-content">
    ...
    <!-- Tables with <caption>, scope="col", scope="row" -->
    <!-- Decorative SVGs with aria-hidden="true" focusable="false" -->
...
Every generated page must meet these accessibility requirements:

- **Skip link**: Add `<a href="#main-content" class="skip-link">Skip to main content</a>` as the first child of `<body>`. Style it hidden until focused (see templates for `.skip-link` CSS).
- **Main landmark**: Wrap all page content in `<main id="main-content">`. This is the skip link target and provides a landmark for screen readers.
- **Color contrast ≥ 4.5:1** for normal text (≤18pt), **≥ 3:1** for large text (≥18pt or ≥14pt bold) and UI components. Always verify `--text-dim` against all surfaces it appears on (`--bg`, `--surface`, `--surface2`). Test with a contrast checker — subtle dim text is the most common failure.
- **Focus-visible styles**: Add a global `:focus-visible` rule: `outline: 2px solid var(--accent); outline-offset: 2px; border-radius: 4px;`. Apply to all interactive elements (buttons, links, inputs, `<summary>`). Never remove focus outlines without a visible replacement.
- **Table semantics**: Every `<table>` needs `<caption>` (use `.sr-only` class if visual caption isn't needed), `scope="col"` on all `<th>` elements, and `scope="row"` on row headers when present.
- **Decorative elements**: Add `aria-hidden="true"` and `focusable="false"` to decorative SVGs, flow arrows, ornamental dots, and crystal overlays. Use `role="img"` with `aria-label` on diagram containers that convey meaning.
- **Minimum font sizes**: No text smaller than 12px. Bump `10px`/`11px` labels, mono badges, and secondary text to `12px` minimum.
- **`prefers-reduced-motion`**: Already required — wrap all animations and transitions. Set `animation-duration: 0.01ms` and `transition-duration: 0.01ms` for reduced motion users.
- **`prefers-contrast: more`**: Add a media query that strengthens borders (`rgba` opacity from ~0.07 to ~0.25), darkens `--text-dim` for higher contrast, and adds visible borders to status badges.
- **Zoom buttons & toolbars**: Mermaid zoom controls need `role="toolbar"`, `aria-label="Diagram zoom controls"`, and individual `aria-label` on each button. Minimum touch target: 24×24px (WCAG 2.5.8).
- **Slide decks**: Deck dots navigation must use `<nav aria-label="Slide navigation">`, each dot gets `aria-label="Go to slide N"` and `aria-current="step"` for the active slide. Add an `aria-live="polite"` region that announces slide transitions. Use `.sr-only` class for the live region.
- **`.sr-only` utility**: Include this class in every page that needs visually-hidden accessible text:
  ```css
  .sr-only {
