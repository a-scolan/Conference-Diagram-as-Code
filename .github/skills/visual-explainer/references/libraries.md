...
**Line breaks in node labels: ALWAYS use `<br/>`, NEVER use `\n`.** Mermaid renders inside HTML — `\n` is silently ignored and produces single-line labels. Only `<br/>` (self-closing) creates actual line breaks in rendered SVG text. Example: `A["First line<br/>Second line"]`.
