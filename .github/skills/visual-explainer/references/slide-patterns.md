...
```javascript
function autoFit() {
  // Mermaid SVGs: expand viewBox for breathing room, then let SVG scale naturally
...
Full-viewport hero. Background treatment via gradient, texture, or surf-generated image. 80–120px display type. When a background image is available (e.g. conference branding), embed it as a base64 data URI in a CSS custom property (`--title-bg`) and apply it with `background-size: cover` to fill the viewport while preserving aspect ratio on any screen.

```html
<section class="slide slide--title" style="background-image:var(--title-bg)">
...
```css
/* Background image as CSS custom property (in :root or inline style) */
/* --title-bg: url(data:image/png;base64,...); */
...
**Line breaks in Mermaid node labels: ALWAYS use `<br/>`, NEVER `\n`.** `\n` is silently ignored — only `<br/>` works. Example: `node["Line 1<br/>Line 2"]`.
...
**Auto-fit SVG to container.** Mermaid renders SVGs with fixed dimensions and an inline `max-width` style that keeps diagrams tiny inside large slides. The `autoFit()` function (see above) handles this at runtime by expanding the viewBox by 20px on each side to prevent text clipping, then removing fixed width/height so the SVG scales naturally. Keep the CSS as a belt-and-suspenders fallback:
...
**Mermaid `flowchart` config.** Always set `useMaxWidth: false` and increase spacing in the Mermaid config to prevent node text truncation:
...
.slide--diagram .mermaid .edgeLabel foreignObject {
  overflow: visible !important;
}

.slide--diagram .mermaid .edgeLabel .label {
  white-space: nowrap !important;
  padding: 2px 6px !important;
  background: var(--surface) !important;
}

.slide--diagram .mermaid .node foreignObject {
  overflow: visible !important;
}

.slide--diagram .mermaid .node foreignObject > div {
  overflow: visible !important;
...
### Code-Diff Slide (dual merge-request comparison)

Side-by-side code blocks styled like a merge request / PR diff. Left panel shows "before" (deletion accent), right panel shows "after" (addition accent). Use for before/after comparisons, migration demos, or any side-by-side code contrast.

```html
<section class="slide slide--code-diff">
  <p class="slide__label reveal">Context label</p>
  <h2 class="slide__heading reveal" style="text-align:center;">Before vs After</h2>
  <div class="slide__code-diff reveal">
    <div>
      <div class="slide__code-block slide__code-block--before">
        <span class="slide__code-filename">old-file.xml</span>
        <pre><code><span class="line-ctx">context line</span>
<span class="line-del">- removed line</span>
<span class="line-add">+ changed to this</span>
<span class="line-ctx">more context</span></code></pre>
      </div>
      <p class="slide__diff-caption reveal">Caption for the "before" side.</p>
    </div>
    <div>
      <div class="slide__code-block slide__code-block--after">
        <span class="slide__code-filename">new-file.c4</span>
        <pre><code><span class="line-ctx">context line</span>
<span class="line-add">+ new addition</span>
<span class="line-add">+   "details here"</span></code></pre>
      </div>
      <p class="slide__diff-caption reveal">Caption for the "after" side.</p>
    </div>
  </div>
</section>
```
...
```css
.slide--code-diff { align-items: center; }
.slide__code-diff {
  display: grid; grid-template-columns: 1fr 1fr;
  gap: clamp(12px, 2vw, 24px);
  max-width: 1200px; width: 100%;
}
.slide__code-diff .slide__code-block {
  max-width: none; border-radius: 12px;
  border-left: 4px solid var(--border);
}
.slide__code-diff .slide__code-block--before { border-left-color: var(--red); }
.slide__code-diff .slide__code-block--after  { border-left-color: var(--green); }
.slide__code-diff .slide__code-block--before .slide__code-filename {
  background: var(--red); color: var(--bg);
}
.slide__code-diff .slide__code-block--after .slide__code-filename {
  background: var(--green); color: var(--bg);
}
.slide__code-diff .line-del {
  display: block; color: var(--red);
  background: var(--red-dim);
  margin: 0 calc(-1 * clamp(24px, 4vw, 48px));
  padding: 0 clamp(24px, 4vw, 48px);
}
.slide__code-diff .line-add {
  display: block; color: var(--green);
  background: var(--green-dim);
  margin: 0 calc(-1 * clamp(24px, 4vw, 48px));
  padding: 0 clamp(24px, 4vw, 48px);
}
.slide__code-diff .line-ctx {
  display: block; color: var(--text-dim);
}
.slide__code-diff .slide__diff-caption {
...
Design constraints:
- Max ~8 lines per panel to avoid overflow.
- `--red` / `--green` and their `-dim` variants must be defined in `:root`.
- Falls back to single column on mobile via `grid-template-columns: 1fr`.
- Filenames float with the existing `.slide__code-filename` pattern.
- Use `<span class="line-del">`, `<span class="line-add">`, `<span class="line-ctx">` inside `<code>` for diff highlighting.

### Quote Slide

36–48px with dramatic line-height. Oversized quotation mark as embedded image (preferred) or typographic decoration (fallback). Generous whitespace is the design.

**Image approach (preferred):** Embed a branded quote mark image as a base64 data URI in `--quote-bg` CSS variable. The opening mark uses `transform: rotate(180deg)` to flip the closing-quote image into an opening quote (“). The closing mark renders the image as-is (”), positioned bottom-right via `text-align: right`.
...
<section class="slide slide--quote">
  ...
  <div class="slide__quote-mark slide__quote-mark--close reveal"></div>
</section>
```

```css
/* Set in :root: --quote-bg: url(data:image/png;base64,...); */

.slide--quote {
  justify-content: center;
  align-items: flex-start;
  text-align: left;
  padding: clamp(60px, 10vh, 120px) clamp(80px, 14vw, 240px);
}
...
.slide__quote-mark::before {
  ...
  height: 100%;
  aspect-ratio: 72 / 62; /* match source image proportions */
  background-image: var(--quote-bg);
  background-size: contain;
  background-repeat: no-repeat;
  opacity: 0.85;
  transform: rotate(180deg); /* flip ” into “ */
}
.slide__quote-mark--close {
  text-align: right;
  width: 100%;
  margin-top: -10px;
  margin-bottom: 0;
...
**Text fallback** (when no branded image is available):

```html
<section class="slide slide--quote">
  <div class="slide__quote-mark slide__quote-mark--text reveal">&ldquo;</div>
  <blockquote class="reveal">Quote text here.</blockquote>
  <cite class="reveal">&mdash; Attribution</cite>
  <div class="slide__quote-mark slide__quote-mark--text slide__quote-mark--close reveal">&rdquo;</div>
</section>
```
...
Full-viewport iframe for interactive external HTML content — LikeC4 architecture diagrams, Storybook, Swagger UI, or any self-contained HTML tool. Includes lazy loading, reload/external-open controls, and an error fallback overlay.

**Full documentation:** See `./likec4-embed.md` for the complete pattern (CSS, JS, HTML structure, LikeC4 export/server approaches, browser configuration for `file://` URLs, accessibility requirements).

**Quick reference:**

```html
<section class="slide slide--embed">
  <h2 class="slide__heading reveal">Interactive Demo</h2>
  <div class="embed-wrap reveal">
    <div class="embed-controls">
      <button onclick="reloadEmbed(this)" title="Reload" aria-label="Reload embed">↻</button>
      <button onclick="openEmbedExternal(this)" title="Open externally" aria-label="Open in new tab">↗</button>
    </div>
    <iframe
      data-src="./exports/architecture.html#/project/myapp/view/overview/"
      loading="lazy"
      sandbox="allow-scripts allow-same-origin allow-popups"
      title="Interactive architecture diagram"
    ></iframe>
    <div class="embed-fallback">
      <p>Could not load the embedded content.</p>
      <a href="./exports/architecture.html" target="_blank" rel="noopener">↗ Open directly</a>
    </div>
  </div>
</section>
```
...
**Key behaviors:**
- `data-src` (not `src`) — iframe lazy-loads via IntersectionObserver when the slide scrolls into view
...
### Popup Embed Slide (CSP-Blocked Sites)

Some external services send `Content-Security-Policy: frame-ancestors 'none'` or `X-Frame-Options: DENY`, which prevents **any** iframe embedding — even from `localhost`. There is no client-side workaround. Common examples: GitHub Codespaces (`*.github.dev`), VS Code for the Web (`vscode.dev`), and many OAuth-dependent SaaS tools.

**Detection:** The iframe fails silently, or the browser console shows `Refused to display in a frame because it set 'X-Frame-Options' to 'deny'` / CSP `frame-ancestors` violations.

For these, use a **fullscreen popup on the same screen** instead of an iframe:

```html
<section class="slide slide--embed" id="slide-external-tool">
  <h2 class="slide__heading reveal">Live Demo — External Tool</h2>
  <div class="embed-wrap reveal" id="tool-embed">
    <div class="embed-controls">
      <button onclick="reloadEmbed(this)" title="Recharger" aria-label="Recharger l'iframe">↻</button>
      <button onclick="openPopupFullscreen(this)" title="Ouvrir en popup plein écran" aria-label="Ouvrir dans une fenêtre popup plein écran">↗</button>
    </div>
    <iframe id="tool-iframe" loading="lazy"
      data-src="https://example.github.dev/"
      ...
      title="External tool — live demo"
    ></iframe>
    <div class="embed-fallback">
      <p style="font-size:18px;">Ce contenu ne peut pas se charger en iframe.</p>
      <p style="font-size:14px; max-width:500px;">Le service bloque l'intégration iframe (CSP). Ouvrez-le dans une fenêtre séparée :</p>
      <button onclick="openPopupFullscreen(this.closest('.embed-wrap').querySelector('.embed-controls button[title*=popup]'))" class="embed-fallback__btn">
...
**JavaScript — fullscreen popup (same screen):**

```js
var externalPopup = null;
function openPopupFullscreen(btn) {
  var wrap = btn.closest('.embed-wrap') || btn.closest('.slide');
  var iframe = wrap ? wrap.querySelector('iframe') : null;
  var url = iframe ? (iframe.src || iframe.dataset.src) : null;
  if (!url) return;

  var w = screen.availWidth;
  var h = screen.availHeight;
  var left = window.screenX;   // same monitor as the presentation
  ...
  if (externalPopup && !externalPopup.closed) {
    ...
    externalPopup = window.open(url, 'external-popup', features);
  }
}
```
...
| Aspect | Why |
|--------|-----|
| `screen.availWidth` × `screen.availHeight` | Fills the screen minus OS chrome (taskbar) |
| `window.screenX` / `window.screenY` | Positions popup on the **same physical screen** as the presentation — critical for multi-monitor setups |
| Singleton guard (`externalPopup.closed`) | Clicking the button again focuses the existing popup instead of opening duplicates |
| Fixed window name (`'external-popup'`) | Browser reuses the same popup window across clicks |
| `location=yes` | Address bar stays visible — required by most browsers, and useful for the audience to see the URL |

**When to use:** GitHub Codespaces, VS Code for the Web, any SaaS that blocks iframing, or when the external service needs its own cookies/session. **When NOT to use:** If the service allows iframe embedding (LikeC4 exports, Storybook, Swagger UI, local HTML) — use the standard Embed Slide above.
...
- Electric Navy: "dark navy conference stage with bold diagonal light rays in electric yellow-lime (#fbfe7a), technical geometric abstract, high contrast, 16:9"
...
Very dark indigo-navy, Poppins Black headings, electric yellow-lime accent. High-contrast conference-stage energy. Dark-first. Faithfully reproduces the **OnePoint Atlantique Day** presentation template.

**Signature visual element — crystal geometric decorations:** Semi-transparent, overlapping tall angular parallelogram shapes on the left and right edges of slides. These create a "faceted crystal" or "light-through-prism" effect. They are rendered as SVG polygons with semi-transparent fills (`rgba(18,28,70,0.5)` to `rgba(25,40,90,0.35)`) injected via JavaScript into each slide. 3–4 shapes per side, spanning most of the slide height. This is the most recognizable visual fingerprint of the Atlantique Day template and MUST be present on title, content, and divider slides.

**No footer bar.** The Atlantique Day template has no visible footer. Do NOT add `.slide-footer`, `::after` bottom bars, or any persistent chrome at the bottom of slides.

**No accent bars on dividers.** Section dividers use only the ghost number + heading. No colored left-side bars.

```css
:root {
  --font-body: 'Poppins', system-ui, sans-serif;
  --font-mono: 'JetBrains Mono', 'SF Mono', monospace;
  --bg: #080d2a;              /* Very dark navy-indigo — much darker than generic navy */
  --surface: #0c1238;
  --surface2: #101848;
  --surface-elevated: #162058;
  --border: rgba(251, 254, 122, 0.06);
  --border-bright: rgba(251, 254, 122, 0.12);
  --text: #e8f0f8;
  --text-dim: #8a9ab8;
  --accent: #fbfe7a;          /* Electric yellow-lime */
  --accent-dim: rgba(251, 254, 122, 0.08);
  --accent2: #e97132;
  --accent2-dim: rgba(233, 113, 50, 0.08);
  --accent3: #0f9ed5;
  --accent3-dim: rgba(15, 158, 213, 0.08);
  --quote-mark: #4a8fa0;      /* Teal/steel-blue for decorative quote marks */
  --code-bg: #060820;
  --code-text: #cde5f5;
  --green: #4ea72e;
  --green-dim: rgba(78, 167, 46, 0.10);
  --red: #e97132;
  --red-dim: rgba(233, 113, 50, 0.10);
  --blue: #0f9ed5;
  --blue-dim: rgba(15, 158, 213, 0.10);
  --split-light: #d0d0d5;     /* Light gray for split panel secondary */
}
@media (prefers-color-scheme: light) {
  :root {
    --bg: #f0f4ff;
    ...
    --border: rgba(8, 13, 42, 0.08);
    --border-bright: rgba(8, 13, 42, 0.14);
    --text: #080d2a;
    --text-dim: #3a4a68;
    --accent: #080d2a;
    --accent-dim: rgba(8, 13, 42, 0.06);
    --accent2: #e97132;
    --accent2-dim: rgba(233, 113, 50, 0.08);
    --accent3: #0f9ed5;
    --accent3-dim: rgba(15, 158, 213, 0.08);
    ...
    --code-text: #cde5f5;
    --green: #196b24;
    --green-dim: rgba(25, 107, 36, 0.08);
    --red: #c0400a;
    --red-dim: rgba(192, 64, 10, 0.08);
    --blue: #0d6fa0;
    --blue-dim: rgba(13, 111, 160, 0.08);
    --split-light: #e8e8ec;
  }
}
```

**Crystal decorations (JavaScript injection).** Add this to the `<script>` block so every slide gets the signature geometric shapes:
...
And the CSS for `.slide-crystals`:
...
**Title slides:** Poppins Black (`font-weight: 900`) display text in `--accent` (#fbfe7a) at maximum scale. **Centered layout** — both horizontally and vertically. Background is flat `--bg` color; the crystal decorations provide visual interest. No diagonal bands, no corner SVGs, no side panels.

**Section dividers:** Large ghost section number in `--accent` at `opacity: 0.06`. Heading in `--text`. Simple accent-line below the heading. No colored left-side bars — the crystal decorations provide enough visual weight.

**Content slides:** "Catégorie" label in `--accent` at top-left. Title in `--accent` bold. Body text in `--text` (white, not dim). Image or illustration on the right side. Background is `--bg` — the crystal decorations are subtly visible.

**Quote slides:** Decorative quote marks via `--quote-bg` image (preferred) or typographic `"` in `--quote-mark` (teal/steel-blue `#4a8fa0`) as fallback. Opening mark top-left with `rotate(180deg)`, closing mark bottom-right, both at roughly 50% opacity. Quote text is **bold white** (`font-weight: 900`, `color: #ffffff`), NOT lime/accent colored. Citation right-aligned below in italic `--text-dim`. Zero resemblance to the typical lime-accent approach — this is one of the most distinctive aspects.

**Split slides:** Left panel dark navy (`--bg`). Right panel **light gray** (`--split-light: #d0d0d5`), typically a placeholder for an image or illustration. 50/50 column split. No decorative borders between panels.

**Three-color semantic system:** Use `--accent2` (orange `#e97132`) for warnings/changes, `--accent3` (cyan `#0f9ed5`) for new features/additions, `--accent` (yellow-lime `#fbfe7a`) for key highlights.

**Typography weight:** Poppins 900 for display, 700 for headings, 400 for body, 300 for captions.

**Image prompt (title slide):** "very dark indigo-navy background with semi-transparent crystalline geometric angular shapes on left and right edges, faceted prism effect, abstract, corporate conference, 16:9"
