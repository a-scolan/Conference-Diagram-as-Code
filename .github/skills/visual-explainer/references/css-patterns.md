...
Reusable patterns for layout, connectors, theming, accessibility, and visual effects in self-contained HTML diagrams.

## Accessibility Foundations

Every generated page must include these accessibility patterns. Place them early in the `<style>` block.
...
Never remove focus outlines without a visible replacement. For buttons that already have borders, increase outline-offset to avoid overlap.
...
Use for table captions, live regions, and supplementary text that should be read but not seen.
...
@media (prefers-contrast: more) {
  :root {
    ...
    --border-bright: rgba(0, 0, 0, 0.40);
    /* Darken --text-dim for stronger contrast */
  }
}
...
- UI components (borders, icons, form controls): minimum **3:1** ratio

Always verify `--text-dim` against **every** surface it appears on — `--bg`, `--surface`, `--surface2`. Subtle dim text on light backgrounds is the most common contrast failure.
